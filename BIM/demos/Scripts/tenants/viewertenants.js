...
// rooms alias label holder as in { id: labelinfo }
var _tenantRoomsLabels = {};
...
var _tenantsNameMap;
// map with tenant names used as source in dropdown autocomplete
var _tenantSearchNameMap;
...
var _MSG_PUBLICFACILITY = 'Public Facility';
var _MSG_TENANTDIRECTORY = 'Company Directory';
var _MSG_FOUNDOTHERLEVELS = 'Found on levels';
var _MSG_COMPANY = 'Company';
var _MSG_UNITID = 'Unit ID';
var _MSG_UNITS = 'Units';
var _MSG_NAME = 'Name';
var _MSG_DETAILSOFTENANT = 'Details of tenant';
var _MSG_SEARCHALL = 'Search by tenant, FM ID or Revit ID';
var _MSG_SEARCHTENANT = 'Search';
var _MSG_SEARCH = 'Search';
var _MSG_EXAMPLECOMPANY = 'Example company';
...
var viewfilter = {
    arch: {
        ...
        namekeys: ['Wall', 'Window', 'Stair', 'Door', 'Rail', 'Generic', 'Curtain', 'Furniture', 'Elevator', '门', '窗', 'Site', '电梯'],
        nodes: null
    },
    str: {
    ...
    fs: {
        ...
        namekeys: ['Fire', 'Sprinklers', 'Security', '火', '安全', '专用设备', '喷淋头'],
        egress: ['Security Appliance', '安全设备'],
        nodes: null
    },
    rooms: {
...
var tenantRoomsMetaEdit = {
    ...
    unitsalias: { name: 'Rooms Alias', type: 'textbox' },
    area: { name: 'Area (SQM)', type: 'label' },
...
function onSetProjectData(data, textStatus) {
    // catch for unhighlight during search without a model loaded
    VA3C = {};
    VA3C.lastObject3D = null;

    // get contracts for all levels
    fillFullTenantToomsTable();

    initSearchUI();
    // see also:
    //initSearchUIKeyboard();
}
...
function parseObjectPropertyFilters(result) {
    ...
    var rows;
    if (result.indexOf('\t') !== -1) {
        // NOTE: when txt file is not UTF-8 but UTF-8 w/BOM,
        //       transfer to linux server converts the file to UCS2-LE BOM and
        //       in 'result' the newlines are missing resulting in wrong parsing
...
function initUIMode() {
    ...
    //$('#extraLabel').click(function (e) {
    //    e.preventDefault();
    //    openTenantProfile($(this));
    //});
...
var keyboardDisplay;//in case undefined
...
function initSearchUI() {
    ...
    $("#fm_search_val")
        ...
        .autocomplete({
            ...
            source: function (request, response) {
                // you can change source '_tenantSearchNameMap' dynamically here to another array
                response(filterSearchOptionsList(_tenantSearchNameMap, (request.term || '').trim().toLocaleLowerCase()));
            },
            select: function (event, option) {
                event.preventDefault();
                $(this).blur();
                var searchtxt = (option.item.value || '').trim();
                if (searchtxt === '' || searchtxt === _MSG_NOMATCH) {
                    $('#fm_search_fb').html('&nbsp;');
                    return;
                }
    ...
    $("#fm_search_button").click(function (e) {
        e.preventDefault();
        searchFMInfo(($("#fm_search_val").val() || '').trim());
        //$("#fm_search_val").select();
        //$("#fm_search_val").focus();
    });
}
function initSearchUIKeyboard() {

    //if (PROJECT.canEdit) {
    //    document.getElementById('fm_search_val').placeholder = '&#xf002; ' + _MSG_SEARCHALL;
    //}
    //else {
    //    document.getElementById('fm_search_val').placeholder = '&#xf002; ' + _MSG_SEARCHTENANT;
    //}

    // FM search
    $('#fm_search_val')
        .attr('placeholder', _MSG_SEARCH) // to add a looking glass add &#xf002; but does not work: must be hard set in html so leave it
        .keyboard({
            ...
            //    console.log('aa', $(keyboard.el).data());
            //    $(keyboard.el).val(el.value);
            //    //$(keyboard.el).autocomplete('search', '');
            //    searchFMInfo(el.value);
            //}
        })
        .autocomplete({
            minLength: 0, // 0 = it shows straight away without typing anything, 1 = show after 1 char
            source: function (request, response) {
                console.log('update');
                response(filterSearchOptionsList(_tenantSearchNameMap, (request.term || '').trim().toLocaleLowerCase()));
            },
            select: function (event, option) {
                ...
                $(this).keyboard().getkeyboard().close(true);
                //$(this).blur();
                //$(this).val(value);
                searchFMInfo(value);
            }
...
function toggleEgress() {
    if (_egressObjects.length === 0) {
        alert('No egress route specified');
        //$("#egressButton").hide();
    }
    ...
    if (vis) {
        ...
        showActiveRooms() || showAllAssignedRooms();
    }

    forceRender();
}

function filterSearchOptionsList(allItems, searchTerm) {

    var matchingItems = {};

    // always filter because list has duplicate values otherwise
    for (var key in allItems) {
        if (searchTerm === '' || key.indexOf(searchTerm) !== -1) {
            matchingItems[allItems[key]] = true;
        }
    }

    matchingItems = Object.keys(matchingItems);
...
function getObjectDisplayUserData(object) {
    ...
    if (PROJECT.canEdit) {
        ...
        userData[_MSG_NAME] = displayname;
        return userData;
    }
    ...
    if (isPublicRoom(object)) {
        var userData = {};
        userData[_MSG_UNITID] = displayname;
        userData[_MSG_PUBLICFACILITY] = _MSG_YES;
        //$.extend(userData, object.userData);
        // no area for GP
    ...
    if (!model) {
        userData[_MSG_COMPANY] = _MSG_NOTENANT;
        userData[_MSG_UNITID] = displayname;
        //$.extend(userData, object.userData);
    ...
    userData[_MSG_COMPANY] = model.Tenant.DisplayName;
    var roomsinfo = getTenantRoomsInfo(model);
    userData[_MSG_UNITS] = roomsinfo[0];
    //$.extend(userData, object.userData);
    //deleteUserDataProp(userData, 'Name');
    //deleteUserDataProp(userData, 'Area');
    return userData;
...
function filterObject(object, modeldata) {
    ...
    for (var f in viewfilter) {
        ...
        for (var k = 0; k < viewfilter[f].namekeys.length; k++) {
            ...
            // egress objects
            if (viewfilter[f].key === viewfilter.fs.key) {
                if (viewfilter.fs.egress) {
                    for (var ei in viewfilter.fs.egress) {
                        if (objectname.indexOf(viewfilter.fs.egress[ei]) !== -1) {
                            _egressObjects.push(object);
                            break;
                        }
                    }
                }
...
function filterObjectUserData(object, filter) {
    ...
    if (viewfilter[filter].key === viewfilter.rooms.key) {
        // public room info
        if (isPublicRoom(object)) {
            userDataUsed[_MSG_PUBLICFACILITY] = "Yes";
        }
...
function openLevelProject(modelname, searchVal) {
    ...
    if (true) {
        // hide to avoid multi click
        //hideLeftRightMenu();
        $('#maincontainer').hide();
        loadProject(PROJECT.id, modelname, searchVal);
    }
}
    ...
    if (matchFilterOnce) {
        if (object3Dcount == toberemoved.length) {
            ...
            for (var ii = toberemoved.length - 1; ii >= 0; ii--) {
                if (_egressObjects.indexOf(toberemoved[ii]) == -1) {
                    removeObject(toberemoved[ii]);
                    //console.log('[INFO] removed object:', toberemoved[ii]);
                    rc++;
                }
            }
            console.log('[INFO] # invalid objects that have been removed: ', rc);
        }
    }
}
...
function clickHandler(event) {
    ...
    console.log('[INFO] select with filter', _currentFilterKeys);
...
function createAnnotations() {
    ...
    $.each(nodes, function (i, object) {
        var ispublic = object.runtimeInfo.hasOwnProperty('publicroom');
        // kiosk: show only public ?
        //if (PROJECT.canEdit || ispublic) {
        {
            ...
            var ispublic = object.runtimeInfo.hasOwnProperty('publicroom');
            //var css = ispublic ? 'anngrey' : null;
            createAnnotation(object.id, objectbox, anntext, annholder);
        }
    });
}
