...
(function ($) {// jscs:ignore requireNamedUnassignedFunctions
    ...
    function getPropertyRowHtml(pgId, name, value, meta, postCreateInitFuncs, getValueFuncs) {
        ...
        if (type === 'boolean' || (type === '' && typeof value === 'boolean')) {
        ...
        else if (type === 'datalist' && Array.isArray(meta.options)) {
            valueHTML = getInputDataListHtml(elemId, value, meta.options, placeholder);
            if (getValueFuncs) {
                getValueFuncs[name] = function () {
                    return document.getElementById(elemId).value;
                    //console.log('shown val', shownVal);
                    //console.log('elm', document.querySelector('#' + elemId + ' option[value="' + shownVal + '"]'));
                    //return document.querySelector('#' + elemId + ' option[value="' + shownVal + '"]').dataset.value;
                    //return $('#' + elemId).val();
                };
            }
        }
        else if (type === 'profiledatalist' && Array.isArray(meta.options)) {
            valueHTML = getInputDataListHtml(elemId, value, meta.options, placeholder, true);
            if (getValueFuncs) {
                getValueFuncs[name] = function () {
                    return document.getElementById(elemId).value;
                    //console.log('shown val', shownVal);
            ...
            valueHTML = '<a href="' + (meta.description || value) + '" target="_blank">' + value + '</a>';
        }
        else if (type === 'openproject') {
            valueHTML = '<div class="tmLink" onclick="openLevelProject(\'' + value + '\', \'' + meta.url + '\');">' + value + '</div>';
            // text box
        }
        else if (type === 'openprojectsearch') {
            valueHTML = '<div class="tmLink" onclick="openLevelProject(\'' + meta.levelid + '\', \'' + meta.name + '\');">' + value + '</div>';
            // text box
        }
        else if (type === 'textbox') {
            valueHTML = '<input type="text" id="' + elemId + '" value="' + value + '" placeholder="' + placeholder + '"></input>';
            if (getValueFuncs) {
                getValueFuncs[name] = function () {
                    return $('#' + elemId).val();
                };
            ...
            var dates = getDateRange(value);
            valueHTML = '<input type="text" class="date" id="dfrom_' + elemId + '" value="' + dates[0] + '" placeholder="' + placeholder + '"></input>' +
                        '&nbsp;>&nbsp;' +
                        '<input type="text" class="date" id="dto_' + elemId + '" value="' + dates[1] + '" placeholder="' + placeholder + '"></input>';

            if (getValueFuncs) {
                getValueFuncs[name] = function () {
                    return ($('#dfrom_' + elemId).val() || '') + SEP_ROOMIDS + ($('#dto_' + elemId).val() || '');
                };
            }
            // room assignment (tr-hover and add/remove button)
        }
        else if (type === 'roomassign') {
            valueHTML = '<div class="left" style="width: 100%; overflow: auto; word-wrap: break-word;">' + value + '</div>' +
                        '<input type="hidden" class="roomscolor left" value="' + meta.color + '" />' +
                        '<div class="tmButton right" onclick="startAssignTenantRooms($(this));">[Add/Remove]</div>';
            // rooms (readonly with tr-hover)
        }
        else if (type === 'rooms' || type === 'roomsopen') {
            valueHTML = '<div style="width: 100%; overflow: auto; word-wrap: break-word;">' + value + '</div>' +
                        '<div class="tenantcolor" style="background-color:' + meta.color + '"></div>';
            // profile (readonly with icon on right)
        }
        else if (type === 'profile') {
            valueHTML = '<span>' + value + '</span>' +
                        '<span class="tmButtonProfile right" onclick="openTenantProfile($(this));"></span>';
        }
        ...
        if (type === 'openproject' || type === 'openprojectsearch') {
            return '<tr class="pgRow"><td class="pgCell">' + displayName + '</td><td class="pgCell">' + valueHTML + '</td></tr>';
        }
        else if (type === 'roomassign' || type === 'rooms' || type === 'roomsopen') {
            // hover class 'roomsrow'
            var rowclass = 'roomsrow';
            if (type === 'roomsopen') rowclass += ' roomsopen'; //open details section onclick
            return '<tr class="pgRow ' + rowclass + '"><td class="pgCell">' + displayName + '</td><td class="pgCell">' + valueHTML + '</td></tr>';
        }
        else {
            return '<tr class="pgRow"><td class="pgCell pc1">' + displayName + '</td><td class="pgCell pc2">' + valueHTML + '</td></tr>';
        }
    }
    ...
    function getInputDataListHtml(id, selectedValue, options, placeholder, addprofile) {
        id = id || '';
        selectedValue = selectedValue || '';
        options = options || [];
        ...
        html += ' class="tenantselect"';
        html += ' placeholder="' + placeholder + '"';
        html += ' list="dl_' + id + '"';
        html += ' value="' + selectedValue + '"';
        html += '/>';
        html += '<datalist id="dl_' + id + '">';
        ...
        for (var i = 0; i < options.length; i++) {
            value = typeof options[i] === 'object' ? options[i].value : options[i];
            text = typeof options[i] === 'object' ? options[i].text : options[i];
            html += '<option data-value="' + value + '" value="' + text + '"' + (selectedValue === text ? ' selected>' : '>');
            html += text + '</option>';
        }
        ...
        if (addprofile) {
            html += '<div class="tmButtonProfile right" onclick="openTenantProfile($(this));"></div>';
        }
