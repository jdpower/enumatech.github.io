...
function initVA3C() {
    ...
    VA3C.highlightMaterial = new THREE.MeshLambertMaterial({ color: 'gold', side: '2' }); //, opacity: 0.5, transparent: true });   //color for selected mesh element
...
function resetCamera() {
    // FIXME: this messes up clipping planes
    //VA3C.controls.reset();
    //instead set target and angle via position and call zoomExtents

    setView(VA3C.defaultCameraAngle);
}

function setView(viewVector) {
    ...
    // set camera angle
    VA3C.controls.object.position.fromArray(viewVector);
...
function zoomExtents() {
    ...
    zoomTo(VA3C.sceneExtents.center, VA3C.sceneExtents.radius, true);
}

function zoomToObject(object, forcezoom) {
    if (object == VA3C.model) {
        zoomExtents();
        return;
    }
    var objectBox = new THREE.Box3().setFromObject(object);
    zoomTo(objectBox.center(), objectBox.size().length() * 0.7, forcezoom, true);
}
...
function updateCameraVerticalAngle() {
    var campos = VA3C.controls.object.getWorldPosition();
    var targetdist = VA3C.controls.target.distanceTo(campos);
    var vertDiff = Math.abs(campos.y - VA3C.controls.target.y);
    // view angle factor (0 = horizontal, 1 = top)
    VA3C.CAMVERTANGLEFACT = Math.asin(vertDiff / targetdist) / HPI;
}
...
function updateAnnotationColor(id, forecolor, backcolor) {
    ...
    if (!anndata) return;
    anndata.element.style.color = forecolor || 'black';
    anndata.element.style.backgroundColor = backcolor || '#ccc';
...
function filterAnnotation(position, loddist) {
    ...
    var dist = position.distanceTo(VA3C.camera.position) * (1.0 - VA3C.CAMVERTANGLEFACT);
...
function openDebugGUI() {
    ...
    if (!VA3C.gui) {
        ...
        VA3C.gui.add(VA3C.settingsAO, "AO enabled").onChange(setAmbientOcclusion);
