...
function fillFullTenantToomsTable() {
    ...
    _todoFillLevels = Object.keys(PROJECT.data.levels);
...
// filter current table and pass to same function as remote GET
function findLocalTenantByName2(tenantname) {
    var filteredModels = [];
    for (var i in _currentTenantRoomsTable) {
        var model = _currentTenantRoomsTable[i];
        if (model.Tenant.DisplayName.indexOf(tenantname) != -1) {
            filteredModels.push(model);
        }
    }
...
function populateTenantOptionsList(result) {
    ...
    $.each(result, function (i, tenantModel) {
        ...
        _tenantNames.push(tenantModel.DisplayName);
    });
...
function showTenantDirectory() {
    ...
    var title = $('<span id="fm_tenant_directory_title" class="panelmenutitle">Tenant Directory (' + _currentLevelModel.DisplayName + ')</span>').appendTo(parentelm);
...
function showFMObjectInfoByRevitId(revit_id) {
    ...
    if (PROJECT.canEdit) {
        ...
        if (isroom) objectInfoMetaEdit.fmid.name = 'Unit ID';
        ...
        entry.jqPropertyGrid({
            ...
            fmid: facility_id,
            remarks: remarks,
            comments: ispublicroom ? _UI_PUBLICFACILITY : ''
        },
        objectInfoMetaEdit);
    }
    else {

        if (isroom) objectInfoMeta.fmid.name = 'Unit ID';
...
function showFMTenantsInfoByModelsOtherLevels(tenantroomsmodels) {
    ...
    var title = $('<span id="fm_search_result_tenant_other_title" class="panelmenutitle">Found on other levels</span>').appendTo(parentelm);
...
function stopRoomAssignment($elm, $root) {
    ...
    $root.css({});
    $elm.html(_UI_ADDREMOVEROOMS).css({});
...
function setAssignRoomsUI($elm, $root) {
    ...
    $elm.html(_UI_PRESSESCASSIGNMENT).css({ color: 'red' });
    $('.panelsectionfeedback').html('&nbsp;').css({});
...
function updateRevitIdToFmIdMap(revit_id, model) {
    ...
    if (!model.FacilityId || model.FacilityId == '') {
        ...
        //console.log('added FM ', model.FacilityId);
    }
}
...
function showAllAssignedRooms(forceUpdate) {
    if (_currentFilterKeys.indexOf(viewfilter.rooms.key) == -1) return;
    for (var i in _currentTenantRoomsTable) {
        showTenantRooms(_currentTenantRoomsTable[i], forceUpdate);
    }
}
...
function showActiveRooms() {
    ...
    if (_activeTenantRoomsModels && _activeTenantRoomsModels.length > 0) {
        for (var i in _activeTenantRoomsModels) {
            showTenantRooms(_activeTenantRoomsModels[i]);
        }
    }
}
...
function getTenantRoomsInfo(model) {
    ...
    roomNames.sort();
    return [roomNames.join(', '), Math.round(totarea * 100) / 100];
}
...
function openTenantProfile($elm) {
    ...
    _panelProfile = $.jsPanel({
        ...
        headerTitle: 'Details of tenant ' + model.Tenant.DisplayName,
        contentSize: { width: 450, height: 240 },
